...
def get_visulization_html_page(request: Request):
    ...
    return templates.TemplateResponse(
        "visualization.html", {"request": request, "backend_url": backend_url}
    )
...
def get_price_distribution_visualization(
    ...
    html_plot = plots.price_distribution(
...
def get_historical_price_distribution_visualization(
    ...
    html_plot = plots.historical_price_trends(
        query_params=query_params, db_session=db_session
    )
    return HTMLResponse(content=html_plot)
...
def get_property_room_distribution_visualization(
    ...
    html_plot = plots.bedrooms_distribution(
        query_params=query_params, db_session=db_session
    )
    return HTMLResponse(content=html_plot)
...
def get_price_zipcode_scatter_plot(
    ...
    html_plot = plots.price_vs_zipcode_box_plot(
        query_params=query_params, db_session=db_session
    )
    return HTMLResponse(content=html_plot)
...
def get_heatmaps_zipcode(
    ...
    html_plot = heatmaps.heatmaps_zipcode(
        query_params=query_params, db_session=db_session
    )
    return HTMLResponse(content=html_plot)
...
@visualization_router.get(
    "/property/historical-zipcode-heatmaps/", response_class=HTMLResponse
)
def get_historical_heatmaps_zipcode(
    query_params: PropertyQueryParams = Depends(property_query_params),
    db_session: Session = Depends(fetch_db_session),
):
    html_plot = heatmaps.historical_heatmaps_zipcode(
        query_params=query_params, db_session=db_session
    )
    return HTMLResponse(content=html_plot)

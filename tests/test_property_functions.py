...
from sqlalchemy_schemas.property import (
    Property,
    filter_properties,
    Base,
)  # Import the Property class and Base
...
data = [
    Property(
        ...
        price=50000 + i * 1000,
        squarefeet=500 + i * 10,
        price_per_square_feet=(50000 + i * 1000) / (500 + i * 10),
        zipcode="",
        state="",
        city="",
        address="",
    ...
    Property(
        ...
        price_per_square_feet=1000,
        zipcode="",
        state="",
        city="",
        address="",
    )
    for i, (price, squarefeet) in enumerate(
        zip(
...
class TestPropertyFunctions(unittest.TestCase):
    ...
    def test_filter_properties(self):
        ...
        response = filter_properties(
            query_params=None, pagination=pagination, db_session=self.session
        )
    ...
    def test_fetch_filtered_property_outliers_on_price(self):
        # query_params = PropertyQueryParams()
        pagination = PageRequest(page=1, page_size=10)
        response = fetch_filtered_property_outliers_on_price(
            query_params=None, pagination=pagination, db_session=self.session
        )
        self.assertIsInstance(response, PaginatedResponse)
    ...
    def test_fetch_filtered_property_outliers_on_price_per_squarefeet(self):
        # query_params = PropertyQueryParams()
        pagination = PageRequest(page=1, page_size=10)
        response = fetch_filtered_property_outliers_on_price_per_squarefeet(
            query_params=None, pagination=pagination, db_session=self.session
        )
        self.assertIsInstance(response, PaginatedResponse)
        self.assertGreater(len(response.results), 0)
    ...
    def test_calculate_property_statistics(self):
        response = calculate_property_statistics(
            query_params=None, db_session=self.session
        )
        ...
        self.assertAlmostEqual(
            ...
            self.session.query(func.avg(Property.price)).scalar(),
        )
        self.assertAlmostEqual(response.median_price, p50)
        self.assertAlmostEqual(
            response.average_price_per_sqft,
            self.session.query(func.avg(Property.price / Property.squarefeet)).scalar(),
        )
        self.assertEqual(
            response.total_properties, self.session.query(Property).count()
        )
